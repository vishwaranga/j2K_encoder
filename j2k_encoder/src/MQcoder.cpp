...
void MQcoder::push_B(queue <uint8_t> *bit_stream){
	if(L>=0){
		bit_stream->push(B);
	}
...
void MQcoder::bit_Stuffing(queue <uint8_t> *bit_stream) {
	// BP = BP + 1; // OUTPUT B
	push_B(bit_stream);
...
void MQcoder::no_bit_Stuffing(queue <uint8_t> *bit_stream) {
	// BP = BP + 1; // OUTPUT B
	push_B(bit_stream);
...
void MQcoder::ByteOut(queue <uint8_t> *bit_stream) {
	if (B == 0xFF) {
...
void MQcoder::RenormalizationENC(queue <uint8_t> *bit_stream) {
	do {
		A = A << 1;
		C = C << 1;
		CT = CT -1;
...
void MQcoder::CodeMPS(queue <uint8_t> *bit_stream) {
	index = INDEX[CX];
	qe =  Q_e[index];
	A = A - qe;
...
void MQcoder::CodeLPS(queue <uint8_t> *bit_stream) {
	index = INDEX[CX];
	qe =  Q_e[index];
	A = A - qe;
...
void MQcoder::FLUSHregister(queue <uint8_t> *bit_stream) {
	int TempC = C + A;
	C = C | 0x0000FFFF;
	if( C > TempC) {
		C = C - 0x00008000;
...
void MQcoder::run(queue <context> *localqueue, queue <int> *headerqueue, queue <uint8_t> *bit_stream) {
